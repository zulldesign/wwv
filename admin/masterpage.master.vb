...
Partial Class admin_masterpage
    ...
    End Sub
    Sub CheckSessions()
        'If Session("logged") <> "yes" Then Response.Redirect("login.aspx")

        Dim MySess = Request.QueryString("sess")
        If Trim(MySess) = "" Then Response.Redirect("login.aspx")
    ...
    Sub WriteTree()
        ...
        TreeView1.Nodes(NodeNr).ChildNodes(0).NavigateUrl = "article_add.aspx" & MySessURL
        TreeView1.Nodes(NodeNr).ChildNodes.Add(New TreeNode("Delete Article"))
        TreeView1.Nodes(NodeNr).ChildNodes(1).NavigateUrl = "article_delete.aspx" & MySessURL
